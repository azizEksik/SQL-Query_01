...
UPDATE mart_satislar
SET urun_fiyat = urun_fiyat - (urun_fiyat*10)/100
WHERE urun_isim = 'Toyota';

-- 4) Ismi A ile baslayan butun musterilerin urun_fiyatlarini %15 arttirin 

UPDATE mart_satislar
SET urun_fiyat = urun_fiyat + (urun_fiyat*15)/100
WHERE musteri_isim LIKE ('a%');

-- 5) Honda araclarin urun id'sini 50 yapin.
