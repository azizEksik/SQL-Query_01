...
SELECT sirket, calisanlar_sayisi, (SELECT AVG(maas)
								   FROM calisanlar
                                   WHERE sirketler.sirket = calisanlar.sirket) AS ort_maas -- field adını vermek icin AS keywordu kullandik
FROM sirketler;                                   
...
SELECT sirket, (SELECT MAX(maas)
				FROM calisanlar
                WHERE sirketler.sirket = calisanlar.sirket) AS max_maas
FROM sirketler;
...
SELECT sirket, calisanlar_sayisi, (SELECT MAX(maas)
                                   ...
                                   (SELECT MIN(maas)
                                    FROM calisanlar
                                    WHERE sirketler.sirket = calisanlar.sirket) AS min_maas
FROM sirketler;
...
SELECT sirket, calisanlar_sayisi, (SELECT SUM(maas)
								   FROM calisanlar
                                   WHERE sirketler.sirket = calisanlar.sirket) AS top_maas
FROM sirketler; 
